...
const SocialFeatureCard = ({ icon, iconSrc, text, customIcon }: any) => (
  <Card className="w-[366px] h-[106px] bg-brand-05 rounded-[20px] overflow-hidden border border-solid border-[#d1e4fe] flex-shrink-0">
        ...
        {customIcon ? (
          ...
          <div className="w-[45px] h-[45px] rounded-xl overflow-hidden shadow-[inset_1px_1px_6px_#fefefe4c] bg-[linear-gradient(136deg,rgba(21,97,201,1)_0%,rgba(26,121,251,1)_100%)]">
            <div className="relative w-6 h-6 top-2.5 left-[11px]">
              <img
                className={`absolute ${iconSrc && iconSrc.includes('vector-8') ? "w-3.5 h-[17px] top-[3px] left-[5px]" : "w-[22px] h-5 top-px left-px"}`}
                alt="Vector"
                src={iconSrc}
              />
            </div>
...
export const SocialFeaturesSection = (): JSX.Element => {
  ...
  return (
    <div className="flex flex-wrap md:flex-nowrap items-center gap-6 w-full justify-center">
      {socialFeatures.map((feature, index) => (
        <SocialFeatureCard key={index} {...feature} />
      ))}
    </div>
  );
};
