...
export const ProductDisplaySection = (): JSX.Element => {
  const [selected, setSelected] = useState("item-0"); // Tracks the currently selected accordion item
        ...
        {faqItems.map((item, index) => {
          ...
          return (
            <AccordionItem
              key={value}
              value={value}
              className={`mb-6 overflow-hidden rounded-xl transition-all ${
                isSelected
                  ? "bg-black text-white"
                  : "bg-neutral-1 text-black"
              }`}
            >
              <AccordionTrigger
                className={`flex justify-between p-6 font-['Manrope',Helvetica] font-medium text-lg tracking-[-0.36px] leading-normal ${
                  isSelected ? "text-white" : "text-black"
                }`}
              >
                {item.question}
              </AccordionTrigger>
              <AccordionContent className="px-8 py-4 bg-background">
