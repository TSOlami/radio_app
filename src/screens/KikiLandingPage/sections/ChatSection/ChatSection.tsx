...
const ChatHeader = () => (
  <div className="w-full h-[122px] bg-white">
    <div className="absolute w-full h-[47px] top-0 left-0">
      ...
      <img className="absolute w-[77px] h-[13px] top-[19px] right-6" alt="Right side" src="/right-side.png" />
    </div>
    <div className="flex items-center justify-between pt-[61px] px-6">
      <div className="flex items-center gap-1">
        <div className="flex w-12 h-11 items-center justify-center p-3 rounded-[20px]">
          <img className="w-6 h-6" alt="Arrow left" src="/arrow-left-2.svg" />
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-1">
              ...
              <img className="w-[18px] h-[18px]" alt="Badge check" src="/badge-check-2.svg" />
            </div>
      ...
      <div className="flex items-center gap-4">
        <div className="relative w-5 h-5">
          <img className="absolute w-[15px] h-[15px] top-0.5 left-0.5" alt="Vector" src="/vector-18.svg" />
        </div>
        <div className="relative w-5 h-5 rotate-90">
          <img className="absolute w-1 h-[15px] top-0.5 left-2 -rotate-90" alt="Vector" src="/vector-4.svg" />
        </div>
      </div>
    </div>
  </div>
...
const ChatMessage = ({ message }: any) => (
  <div className={`flex flex-col ${message.type === "sent" ? "items-end" : "items-start"}`}>
    <div className={`flex flex-col ${message.type === "sent" ? "items-end" : "items-start"} gap-1`}>
      <Card className={`border-0 shadow-none ${message.type === "sent" ? "bg-[#1a79fb] rounded-[20px_20px_0px_20px]" : "bg-neutral-2 rounded-[20px_20px_20px_0px]"}`}>
        <CardContent className="p-5">
          {message.hasImage && (
            <img className="w-full h-[220px] object-cover mb-2" alt="Rectangle" src="/rectangle-22.svg" />
          )}
          <p className={`mt-[-1.00px] font-body-1-regular font-[number:var(--body-1-regular-font-weight)] ${message.type === "sent" ? "text-white" : "text-[#030c19]"} text-[length:var(--body-1-regular-font-size)] tracking-[var(--body-1-regular-letter-spacing)] leading-[var(--body-1-regular-line-height)] [font-style:var(--body-1-regular-font-style)]`}>
            {message.content}
          </p>
        </CardContent>
      </Card>
      <span className={`font-body-3-regular font-[number:var(--body-3-regular-font-weight)] text-[#4f5966] text-[length:var(--body-3-regular-font-size)] ${message.type === "sent" ? "text-right" : ""} tracking-[var(--body-3-regular-letter-spacing)] leading-[var(--body-3-regular-line-height)] [font-style:var(--body-3-regular-font-style)]`}>
        {message.timestamp}
      </span>
    </div>
  </div>
